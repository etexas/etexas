 ...
 */
/**
 * @class ETexas.view.detector.CreateDetectorFormController
 * @extends ETexas.view.detector.DetectorFormController
 * 
 * The {@link ETexas.view.detector.CreateDetectorForm} controller.
 * 
 * @author emyers
 */
Ext.define('ETexas.view.detector.CreateDetectorFormController', {
    extend : 'ETexas.view.detector.DetectorFormController',
    alias : 'controller.createdetectorform',
    ...
    onCreateButtonClicked : function(button, e, eOpts) {
        ...
        if (view.isValid()) {
            ...
            view.mask('Creating Detector...');
            view.submit({
                scope : this,
                method : 'POST',
                url : ETexas.util.UrlProvider.getDetectorsUrl(selectedComposite, selectedSimulation),
                success : function(form, action) {

                    var detector = Ext.JSON.decode(action.response.responseText);
                    view.fireEvent('detectoradded', detector.id);
                    Ext.destroy(view);
                },
    ...
    onHelpToolClicked : function(tool, e, owner, eOpts) {
        ...
        window.open(ETexas.util.UrlProvider.getHelpContentUrl() + '/Simulations/Simulations/create_detector.htm');
    }
});
