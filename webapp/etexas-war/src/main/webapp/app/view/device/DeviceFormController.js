 ...
 */
/**
 * @class ETexas.view.device.DeviceFormController
 * @extends ETexas.view.model.ModelFormController
 * 
 * The {@link ETexas.view.device.DeviceForm} controller.
 * 
 * @abstract
 * @author emyers
 */
Ext.define('ETexas.view.device.DeviceFormController', {
    ...
    validateNameField : function(value) {
        ...
        if (value.length === 0) {
            ...
            return 'A valid device name is required.';
        }

        var deviceStore = this.getViewModel().get('deviceStore');

        if (deviceStore.query('name', value, false, false, true).length !== 0) {

            return 'A device with the name \"' + value + '\" already exists in the composite.';
        }
