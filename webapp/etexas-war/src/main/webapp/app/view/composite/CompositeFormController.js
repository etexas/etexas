 ...
 */
/**
 * @class ETexas.view.composite.CompositeFormController
 * @extends ETexas.view.model.ModelFormController
 * 
 * The {@link ETexas.view.composite.CompositeForm} controller.
 * 
 * @abstract
 * @author emyers
 */
Ext.define('ETexas.view.composite.CompositeFormController', {
    ...
    validateNameField : function(value) {
        ...
        if (value.length === 0) {
            ...
            return 'A valid composite name is required.';
        }

        var compositeStore = this.getViewModel().get('compositeStore');

        if (compositeStore.query('name', value, false, false, true).length !== 0) {

            return 'A composite with the name \"' + value + '\" already exists.';
        }
