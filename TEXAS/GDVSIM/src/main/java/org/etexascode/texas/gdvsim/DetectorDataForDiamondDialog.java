...
/******************************************************************************/
/*                    DetectorDataForDiamondDialog.java                       */
/******************************************************************************/
/*                                                                            */
...
class DetectorDataForDiamondDialog extends JDialog {
    ...
    public DetectorDataForDiamondDialog(JButton preDetectorConnButton) {
        ...
        for (int lsiv_det = 1; lsiv_det <= PARAMS.TEXAS_MODEL_NLS; lsiv_det++) {
            det_classify_num_default[lsiv_det] = lclv_tx_fmt.msia_def[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_CLASS_NUM + lsiv_det];

            for (int lsiv_class = 1; lsiv_class <= PARAMS.TEXAS_MODEL_LDC; lsiv_class++) {
                det_classify_name_default[lsiv_det][lsiv_class] = lclv_tx_fmt.msta_def[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_CLASS_NAM01 - 1 + lsiv_class];
                det_classify_lower_default[lsiv_det][lsiv_class] = lclv_tx_fmt.msia_def[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_CLASS_LL_01 - 1 + lsiv_class];
                det_classify_upper_default[lsiv_det][lsiv_class] = lclv_tx_fmt.msia_def[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_CLASS_UL_01 - 1 + lsiv_class];
            }
        }

        for (int lsiv_det = 1; lsiv_det <= PARAMS.TEXAS_MODEL_NLS; lsiv_det++) {
            if (gdvsim.gclv_inter.mclv_TX_Inter_Data.mclv_tx_sim_data.mclv_det.mcla_det_data[lsiv_det].mclv_aux.msia_stat[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_CLASS_NUM
                ...
                gdvsim.det_classify_num_stat[lsiv_det] = gdvsim.gclv_inter.mclv_TX_Inter_Data.mclv_tx_sim_data.mclv_det.mcla_det_data[lsiv_det].mclv_aux.msia_stat[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_CLASS_NUM];

                for (int lsiv_class = 1; lsiv_class <= PARAMS.TEXAS_MODEL_LDC; lsiv_class++) {
                    gdvsim.det_classify_name_stat[lsiv_det][lsiv_class] = gdvsim.gclv_inter.mclv_TX_Inter_Data.mclv_tx_sim_data.mclv_det.mcla_det_data[lsiv_det].mclv_aux.msia_stat[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_CLASS_NAM01
                            - 1 + lsiv_class];
                    gdvsim.det_classify_lower_stat[lsiv_det][lsiv_class] = gdvsim.gclv_inter.mclv_TX_Inter_Data.mclv_tx_sim_data.mclv_det.mcla_det_data[lsiv_det].mclv_aux.msia_stat[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_CLASS_LL_01
                            - 1 + lsiv_class];
                    gdvsim.det_classify_upper_stat[lsiv_det][lsiv_class] = gdvsim.gclv_inter.mclv_TX_Inter_Data.mclv_tx_sim_data.mclv_det.mcla_det_data[lsiv_det].mclv_aux.msia_stat[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_CLASS_UL_01
                            - 1 + lsiv_class];
                }
        ...
        if (gdvsim.gclv_inter.mbov_is_ic_SEMI_ACT) {
            titleString = "Detector Data For Diamond Semi-Actuated";
        }
        else if (gdvsim.gclv_inter.mbov_is_ic_FULL_ACT) {
            titleString = "Detector Data For Diamond Full-Actuated";
        }
        else if (gdvsim.gclv_inter.mbov_is_ic_NEMA) {
            titleString = "Detector Data For Diamond NEMA";
        }
        else if (gdvsim.gclv_inter.mbov_is_ic_HARDWARE) {
            titleString = "Detector Data For Diamond Hardware-in-the-Loop";
        }
        else {
            titleString = "error";
        }
        ...
        String[] array_DATDIA_LEG = new String[leg_num_has_lane];
        ...
        for (int lsiv_leg = 0; lsiv_leg <= PARAMS.TEXAS_MODEL_NLGP1; lsiv_leg++) {
            ...
            if (lsiv_leg == 0) {
                array_DATDIA_LEG[index++] = "IR";
            }
            else if (lsiv_leg == PARAMS.TEXAS_MODEL_NLGP1) {
                array_DATDIA_LEG[index++] = "IL";
            }
            else {
                array_DATDIA_LEG[index++] = Integer.toString(lsiv_leg);
            }
        }

        for (int lsiv_det = 1; lsiv_det <= PARAMS.TEXAS_MODEL_NLS; lsiv_det++) {
            cbo_det[lsiv_det][DATDIA_LEG] = new JComboBox(array_DATDIA_LEG);
        }

        if (gdvsim.flag_detDataForDiamond_ok) {
            for (int lsiv_det = 1; lsiv_det <= PARAMS.TEXAS_MODEL_NLS; lsiv_det++) {
                if (gdvsim.gclv_inter.mclv_TX_Inter_Data.mclv_tx_sim_data.mclv_det.mcla_det_data[lsiv_det].mclv_aux.msia_stat[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_LEG] != gdvsim.gclv_inter.TX_DATA_IS_INVALID) {
                    if (gdvsim.gclv_inter.mclv_TX_Inter_Data.mclv_tx_sim_data.mclv_det.mcla_det_data[lsiv_det].msiv_leg == 0) {
                        cbo_det[lsiv_det][DATDIA_LEG].setSelectedItem("IR");
                    }
                    else if (gdvsim.gclv_inter.mclv_TX_Inter_Data.mclv_tx_sim_data.mclv_det.mcla_det_data[lsiv_det].msiv_leg == PARAMS.TEXAS_MODEL_NLGP1) {
                        cbo_det[lsiv_det][DATDIA_LEG].setSelectedItem("IL");
                    }
                    else {
                        cbo_det[lsiv_det][DATDIA_LEG].setSelectedItem(Integer.toString(gdvsim.gclv_inter.mclv_TX_Inter_Data.mclv_tx_sim_data.mclv_det.mcla_det_data[lsiv_det].msiv_leg));
                    }
                }
                else {
                    cbo_det[lsiv_det][DATDIA_LEG].setSelectedItem(lclv_tx_fmt.msta_def[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_LEG]);
                }
            }
        }
        else {
        ...
        String[] array_DATDIA_TYPE = new String[4];

        if (gdvsim.gclv_inter.mbov_is_ic_NEMA || gdvsim.gclv_inter.mbov_is_ic_HARDWARE) {
            array_DATDIA_TYPE = lclv_tx_fmt.msta_val[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_TYPE].substring(1).split("\\|");
        }
        else {
            String type = "|IN|PR|PU|";
            array_DATDIA_TYPE = type.substring(1).split("\\|");
        }

        lsiv_min = lclv_tx_fmt.msia_min[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_DELAY];
        lsiv_max = lclv_tx_fmt.msia_max[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_DELAY];
        lsiv_inc = lclv_tx_fmt.msia_inc[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_DELAY];

        count = (lsiv_max - lsiv_min) / lsiv_inc + 1;
        int_number = lsiv_min;
        String[] array_DATDIA_DELAY = new String[count];
        for (int lsiv_i = 0; lsiv_i < count; lsiv_i++) {
            array_DATDIA_DELAY[lsiv_i] = Integer.toString(int_number);
            int_number += lsiv_inc;
        }

        Double lsiv_min_double = lclv_tx_fmt.mdfa_min[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_EXTEND];
        Double lsiv_max_double = lclv_tx_fmt.mdfa_max[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_EXTEND];
        Double lsiv_inc_double = lclv_tx_fmt.mdfa_inc[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_EXTEND];

        count = (int)((lsiv_max_double - lsiv_min_double) / lsiv_inc_double) + 1;
        Double double_number = lsiv_min_double;
        String[] array_DATDIA_EXTEND = new String[count];
        for (int lsiv_i = 0; lsiv_i < count; lsiv_i++) {
            array_DATDIA_EXTEND[lsiv_i] = oneDigits.format(double_number);
        ...
        for (int lsiv_det = 1; lsiv_det <= PARAMS.TEXAS_MODEL_NLS; lsiv_det++) {
            ...
            cbo_det[lsiv_det][DATDIA_DELAY] = new JComboBox(array_DATDIA_DELAY);
            cbo_det[lsiv_det][DATDIA_EXTEND] = new JComboBox(array_DATDIA_EXTEND);
        }
        ...
        if (gdvsim.flag_detDataForDiamond_ok) {
            for (int lsiv_det = 1; lsiv_det <= PARAMS.TEXAS_MODEL_NLS; lsiv_det++) {
                ...
                if (gdvsim.gclv_inter.mclv_TX_Inter_Data.mclv_tx_sim_data.mclv_det.mcla_det_data[lsiv_det].mclv_aux.msia_stat[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_DELAY] != gdvsim.gclv_inter.TX_DATA_IS_INVALID) {
                    cbo_det[lsiv_det][DATDIA_DELAY].setSelectedItem(Integer.toString(gdvsim.gclv_inter.mclv_TX_Inter_Data.mclv_tx_sim_data.mclv_det.mcla_det_data[lsiv_det].msiv_delay));
                }
                else {
                    cbo_det[lsiv_det][DATDIA_DELAY].setSelectedItem(Integer.toString(lclv_tx_fmt.msia_def[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_DELAY]));
                }

                if (gdvsim.gclv_inter.mclv_TX_Inter_Data.mclv_tx_sim_data.mclv_det.mcla_det_data[lsiv_det].mclv_aux.msia_stat[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_EXTEND] != gdvsim.gclv_inter.TX_DATA_IS_INVALID) {
                    cbo_det[lsiv_det][DATDIA_EXTEND].setSelectedItem(oneDigits.format(gdvsim.gclv_inter.mclv_TX_Inter_Data.mclv_tx_sim_data.mclv_det.mcla_det_data[lsiv_det].mdfv_extend));
                }
                else {
                    cbo_det[lsiv_det][DATDIA_EXTEND].setSelectedItem(oneDigits.format(lclv_tx_fmt.mdfa_def[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_EXTEND]));
                }
            }
        }
        else {
        ...
        for (int lsiv_det = 1; lsiv_det <= PARAMS.TEXAS_MODEL_NLS; lsiv_det++) {
            ...
            cbo_det[lsiv_det][DATDIA_TYPE].addActionListener(componentActionListener);
            ...
            cbo_det[lsiv_det][DATDIA_TYPE].addKeyListener(componentKeyListener);
        }
    ...
    void setSize() {
        label_field[DATDIA_LEG].setPreferredSize(new Dimension(55, 120));
        label_field[DATDIA_FL].setPreferredSize(new Dimension(55, 120));
        label_field[DATDIA_NL].setPreferredSize(new Dimension(55, 120));
        label_field[DATDIA_SPA].setPreferredSize(new Dimension(60, 120));
        label_field[DATDIA_LEN].setPreferredSize(new Dimension(55, 120));
        label_field[DATDIA_TYPE].setPreferredSize(new Dimension(65, 120));
        label_field[DATDIA_DELAY].setPreferredSize(new Dimension(60, 120));
        label_field[DATDIA_EXTEND].setPreferredSize(new Dimension(60, 120));
        label_field[NUMOFFIELD].setPreferredSize(new Dimension(55, 120));

        label_field[DATDIA_LEG].setMaximumSize(new Dimension(55, 120));
        label_field[DATDIA_FL].setMaximumSize(new Dimension(55, 120));
        label_field[DATDIA_NL].setMaximumSize(new Dimension(55, 120));
        label_field[DATDIA_SPA].setMaximumSize(new Dimension(60, 120));
        label_field[DATDIA_LEN].setMaximumSize(new Dimension(55, 120));
        label_field[DATDIA_TYPE].setMaximumSize(new Dimension(65, 120));
        label_field[DATDIA_DELAY].setMaximumSize(new Dimension(60, 120));
        label_field[DATDIA_EXTEND].setMaximumSize(new Dimension(60, 120));
        ...
        for (int lsiv_det = 1; lsiv_det <= PARAMS.TEXAS_MODEL_NLS; lsiv_det++) {
            ...
            cbo_det[lsiv_det][DATDIA_LEG].setPreferredSize(new Dimension(55, 25));
            cbo_det[lsiv_det][DATDIA_FL].setPreferredSize(new Dimension(55, 25));
            cbo_det[lsiv_det][DATDIA_NL].setPreferredSize(new Dimension(55, 25));
            cbo_det[lsiv_det][DATDIA_SPA].setPreferredSize(new Dimension(60, 25));
            cbo_det[lsiv_det][DATDIA_LEN].setPreferredSize(new Dimension(55, 25));
            cbo_det[lsiv_det][DATDIA_TYPE].setPreferredSize(new Dimension(65, 25));
            cbo_det[lsiv_det][DATDIA_DELAY].setPreferredSize(new Dimension(60, 25));
            cbo_det[lsiv_det][DATDIA_EXTEND].setPreferredSize(new Dimension(60, 25));

            cbo_det[lsiv_det][DATDIA_LEG].setMaximumSize(new Dimension(55, 25));
            cbo_det[lsiv_det][DATDIA_FL].setMaximumSize(new Dimension(55, 25));
            cbo_det[lsiv_det][DATDIA_NL].setMaximumSize(new Dimension(55, 25));
            cbo_det[lsiv_det][DATDIA_SPA].setMaximumSize(new Dimension(60, 25));
            cbo_det[lsiv_det][DATDIA_LEN].setMaximumSize(new Dimension(55, 25));
            cbo_det[lsiv_det][DATDIA_TYPE].setMaximumSize(new Dimension(65, 25));
            cbo_det[lsiv_det][DATDIA_DELAY].setMaximumSize(new Dimension(60, 25));
            cbo_det[lsiv_det][DATDIA_EXTEND].setMaximumSize(new Dimension(60, 25));
    ...
    void setStatus(int sumOfDetectors) {
        ...
        if (gdvsim.gclv_inter.mbov_is_ic_NEMA || gdvsim.gclv_inter.mbov_is_ic_HARDWARE) {
            for (int lsiv_det = 1; lsiv_det <= PARAMS.TEXAS_MODEL_NLS; lsiv_det++) {
                if (cbo_det[lsiv_det][DATDIA_TYPE].getSelectedItem().toString().equals("CL")) {
    ...
    void setValueAfterAdd(int sum, int index) {
        for (int lsiv_det = (sum + 1); lsiv_det >= index; lsiv_det--) {
            cbo_det[lsiv_det][DATDIA_LEG].removeActionListener(componentActionListener);
            cbo_det[lsiv_det][DATDIA_LEG].removeKeyListener(componentKeyListener);
    ...
    void setValueAfterDel(int sum, int index) {
        ...
        cbo_det[sum][DATDIA_SPA].setSelectedItem(Integer.toString(lclv_tx_fmt.msia_def[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_SPA]));
        cbo_det[sum][DATDIA_LEN].setSelectedItem(Integer.toString(lclv_tx_fmt.msia_def[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_LEN]));
        cbo_det[sum][DATDIA_TYPE].setSelectedItem(lclv_tx_fmt.msta_def[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_TYPE]);
        cbo_det[sum][DATDIA_DELAY].setSelectedItem(Integer.toString(lclv_tx_fmt.msia_def[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_DELAY]));
        cbo_det[sum][DATDIA_EXTEND].setSelectedItem(oneDigits.format(lclv_tx_fmt.mdfa_def[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_EXTEND]));
        ...
        if (firstLeg == 0) {
            cbo_det[sum][DATDIA_LEG].setSelectedItem("IR");
        }
        else if (firstLeg == PARAMS.TEXAS_MODEL_NLGP1) {
            cbo_det[sum][DATDIA_LEG].setSelectedItem("IL");
        }
        else {
            cbo_det[sum][DATDIA_LEG].setSelectedItem(Integer.toString(firstLeg));
        }

        cbo_det[sum][DATDIA_FL].removeAllItems();

        for (int lsiv_lane = 1; lsiv_lane <= number_of_lanes[firstLeg]; lsiv_lane++) {
            cbo_det[sum][DATDIA_FL].addItem(Integer.toString(lsiv_lane));
        }

        int firstLane = Integer.valueOf(cbo_det[sum][DATDIA_FL].getSelectedItem().toString()).intValue();
        int rangeForField3 = number_of_lanes[firstLeg] - firstLane + 1;

        cbo_det[sum][DATDIA_NL].removeAllItems();

        for (int lsiv_lane = 1; lsiv_lane <= rangeForField3; lsiv_lane++) {
            cbo_det[sum][DATDIA_NL].addItem(Integer.toString(lsiv_lane));
        }
    ...
    void setValueAfterUp(int index) {
        cbo_det[index][DATDIA_LEG].removeActionListener(componentActionListener);
        cbo_det[index][DATDIA_LEG].removeKeyListener(componentKeyListener);
        cbo_det[index][DATDIA_FL].removeActionListener(componentActionListener);
        cbo_det[index][DATDIA_FL].removeKeyListener(componentKeyListener);
        cbo_det[index][DATDIA_NL].removeActionListener(componentActionListener);
        cbo_det[index][DATDIA_NL].removeKeyListener(componentKeyListener);

        cbo_det[index - 1][DATDIA_LEG].removeActionListener(componentActionListener);
        cbo_det[index - 1][DATDIA_LEG].removeKeyListener(componentKeyListener);
        cbo_det[index - 1][DATDIA_FL].removeActionListener(componentActionListener);
        cbo_det[index - 1][DATDIA_FL].removeKeyListener(componentKeyListener);
        cbo_det[index - 1][DATDIA_NL].removeActionListener(componentActionListener);
        cbo_det[index - 1][DATDIA_NL].removeKeyListener(componentKeyListener);
        ...
        cbo_det[index][DATDIA_LEG].addActionListener(componentActionListener);
        cbo_det[index][DATDIA_LEG].addKeyListener(componentKeyListener);
        cbo_det[index][DATDIA_FL].addActionListener(componentActionListener);
        cbo_det[index][DATDIA_FL].addKeyListener(componentKeyListener);
        cbo_det[index][DATDIA_NL].addActionListener(componentActionListener);
        cbo_det[index][DATDIA_NL].addKeyListener(componentKeyListener);

        cbo_det[index - 1][DATDIA_LEG].addActionListener(componentActionListener);
        cbo_det[index - 1][DATDIA_LEG].addKeyListener(componentKeyListener);
        cbo_det[index - 1][DATDIA_FL].addActionListener(componentActionListener);
        cbo_det[index - 1][DATDIA_FL].addKeyListener(componentKeyListener);
        cbo_det[index - 1][DATDIA_NL].addActionListener(componentActionListener);
        cbo_det[index - 1][DATDIA_NL].addKeyListener(componentKeyListener);
        ...
        public void keyPressed(KeyEvent event) {
            if (event.getKeyCode() == KeyEvent.VK_F1) {
                ...
                for (int lsiv_leg = 0; lsiv_leg <= (number_of_legs + 1); lsiv_leg++) {
                    for (int lsiv_lane = 1; lsiv_lane <= number_of_lanes[lsiv_leg]; lsiv_lane++) {
                        if (event.getSource() == cbo_table[lsiv_leg][lsiv_lane]) {
                            ...
                            new HelpDialog(true, "Number of Detectors for leg " + legName + " inbound lane " + lsiv_lane, "Number of Detectors for leg " + legName + " inbound lane " + lsiv_lane,
                ...
                for (int lsiv_det = 1; lsiv_det <= PARAMS.TEXAS_MODEL_NLS; lsiv_det++) {
                    if (event.getSource() == add[lsiv_det]) {
                    ...
                    else if (event.getSource() == cbo_det[lsiv_det][DATDIA_LEG]) {
                        ...
                        new HelpDialog(lclv_tx_fmt.mboa_def[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_LEG], lclv_tx_fmt.mstv_name.replace("#", Integer.toString(lsiv_det)),
                                ...
                                lclv_tx_fmt.msta_help[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_LEG], cbo_det[lsiv_det][DATDIA_LEG].getSelectedItem().toString(),
                                lclv_tx_fmt.msta_def[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_LEG], lclv_tx_fmt.msta_val[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_LEG], " ", " ", " ");
                    }
                    ...
                    else if (event.getSource() == cbo_det[lsiv_det][DATDIA_DELAY]) {
                        new HelpDialog(lclv_tx_fmt.mboa_def[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_DELAY], lclv_tx_fmt.mstv_name.replace("#", Integer.toString(lsiv_det)),
                                lclv_tx_fmt.msta_desc[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_DELAY] + " for Detector " + lsiv_det,
                                lclv_tx_fmt.msta_help[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_DELAY], cbo_det[lsiv_det][DATDIA_DELAY].getSelectedItem().toString(),
                                Integer.toString(lclv_tx_fmt.msia_def[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_DELAY]), " ",
                                Integer.toString(lclv_tx_fmt.msia_min[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_DELAY]),
                                Integer.toString(lclv_tx_fmt.msia_max[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_DELAY]),
                                Integer.toString(lclv_tx_fmt.msia_inc[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_DELAY]));
                    }
                    else if (event.getSource() == cbo_det[lsiv_det][DATDIA_EXTEND]) {
                        new HelpDialog(lclv_tx_fmt.mboa_def[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_EXTEND], lclv_tx_fmt.mstv_name.replace("#", Integer.toString(lsiv_det)),
                                lclv_tx_fmt.msta_desc[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_EXTEND] + " for Detector " + lsiv_det,
                                lclv_tx_fmt.msta_help[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_EXTEND], cbo_det[lsiv_det][DATDIA_EXTEND].getSelectedItem().toString(),
                                oneDigits.format(lclv_tx_fmt.mdfa_def[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_EXTEND]), " ",
                                oneDigits.format(lclv_tx_fmt.mdfa_min[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_EXTEND]),
                                oneDigits.format(lclv_tx_fmt.mdfa_max[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_EXTEND]),
                                oneDigits.format(lclv_tx_fmt.mdfa_inc[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_EXTEND]));
                    }
    ...
    void saveClassify() {
        ...
        for (int lsiv_det = 1; lsiv_det <= numOfDetectors; lsiv_det++) {
            if (cbo_det[lsiv_det][DATDIA_TYPE].getSelectedItem().toString().equals("CL")) {
                ...
                gdvsim.gclv_inter.mclv_TX_Inter_Data.mclv_tx_sim_data.mclv_det.mcla_det_data[lsiv_det].mclv_aux.msia_stat[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_CLASS_NUM] = gdvsim.det_classify_num_stat[lsiv_det];
                for (int lsiv_class = 1; lsiv_class <= PARAMS.TEXAS_MODEL_LDC; lsiv_class++) {
                ...
                gdvsim.gclv_inter.mclv_TX_Inter_Data.mclv_tx_sim_data.mclv_det.mcla_det_data[lsiv_det].mclv_aux.msia_stat[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_CLASS_NUM] = gdvsim.gclv_inter.TX_DATA_IS_INVALID;
                for (int lsiv_class = 1; lsiv_class <= PARAMS.TEXAS_MODEL_LDC; lsiv_class++) {
                    gdvsim.gclv_inter.mclv_TX_Inter_Data.mclv_tx_sim_data.mclv_det.mcla_det_data[lsiv_det].mclv_aux.msia_stat[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_CLASS_NAM01 - 1
    ...
    void saveData() {
        ...
        for (int lsiv_det = 1; lsiv_det <= PARAMS.TEXAS_MODEL_NLS; lsiv_det++) {
            gdvsim.gclv_inter.mclv_TX_Inter_Data.mclv_tx_sim_data.mclv_det.mcla_det_data[lsiv_det].mclv_aux.msia_stat[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_LEG] = gdvsim.gclv_inter.TX_DATA_IS_INVALID;
            gdvsim.gclv_inter.mclv_TX_Inter_Data.mclv_tx_sim_data.mclv_det.mcla_det_data[lsiv_det].mclv_aux.msia_stat[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_FL] = gdvsim.gclv_inter.TX_DATA_IS_INVALID;
            gdvsim.gclv_inter.mclv_TX_Inter_Data.mclv_tx_sim_data.mclv_det.mcla_det_data[lsiv_det].mclv_aux.msia_stat[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_NL] = gdvsim.gclv_inter.TX_DATA_IS_INVALID;
            gdvsim.gclv_inter.mclv_TX_Inter_Data.mclv_tx_sim_data.mclv_det.mcla_det_data[lsiv_det].mclv_aux.msia_stat[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_SPA] = gdvsim.gclv_inter.TX_DATA_IS_INVALID;
            gdvsim.gclv_inter.mclv_TX_Inter_Data.mclv_tx_sim_data.mclv_det.mcla_det_data[lsiv_det].mclv_aux.msia_stat[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_LEN] = gdvsim.gclv_inter.TX_DATA_IS_INVALID;
            gdvsim.gclv_inter.mclv_TX_Inter_Data.mclv_tx_sim_data.mclv_det.mcla_det_data[lsiv_det].mclv_aux.msia_stat[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_TYPE] = gdvsim.gclv_inter.TX_DATA_IS_INVALID;
            gdvsim.gclv_inter.mclv_TX_Inter_Data.mclv_tx_sim_data.mclv_det.mcla_det_data[lsiv_det].mclv_aux.msia_stat[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_DELAY] = gdvsim.gclv_inter.TX_DATA_IS_INVALID;
            gdvsim.gclv_inter.mclv_TX_Inter_Data.mclv_tx_sim_data.mclv_det.mcla_det_data[lsiv_det].mclv_aux.msia_stat[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_EXTEND] = gdvsim.gclv_inter.TX_DATA_IS_INVALID;
            gdvsim.gclv_inter.mclv_TX_Inter_Data.mclv_tx_sim_data.mclv_det.mcla_det_data[lsiv_det].mclv_aux.msia_stat[gdvsim.gclv_inter.TX_FMT_SIM_DETECT_DATDIA_CLASS_NUM] = gdvsim.gclv_inter.TX_DATA_IS_INVALID;
        }
        ...
        for (int lsiv_det = 1; lsiv_det <= numOfDetectors; lsiv_det++) {
            ...
            gdvsim.gclv_inter.mclv_TX_Inter_Data.mclv_tx_sim_data.mclv_det.mcla_det_data[lsiv_det].msiv_delay = Integer.valueOf(cbo_det[lsiv_det][DATDIA_DELAY].getSelectedItem().toString())
                    .intValue();
            gdvsim.gclv_inter.mclv_TX_Inter_Data.mclv_tx_sim_data.mclv_det.mcla_det_data[lsiv_det].mdfv_extend = Double.valueOf(cbo_det[lsiv_det][DATDIA_EXTEND].getSelectedItem().toString())
                    .doubleValue();
            gdvsim.gclv_inter.mclv_TX_Inter_Data.mclv_tx_sim_data.mclv_det.mcla_det_data[lsiv_det].msiv_class_num = gdvsim.det_classify_num[lsiv_det];
        ...
        public void actionPerformed(ActionEvent event) {
            if (event.getSource() == okButton || event.getSource() == applyButton) {
                if (!isError()) {
                    ...
                    gdvsim.flag_detDataForDiamond_ok = true;
                    saveData();
